...
\begin{align}
    \mathcal{J}(\theta) &= \frac{1}{2}\mathbb{E}_{p_{\text{data}}(x)} \left[ \left\| s_\theta(x) - \nabla_x \log p_{\text{data}}(x) \right\|^2 \right]
\end{align}
...
\begin{align}
    \mathcal{J}(\theta) &= \mathbb{E}_{p_{\text{data}}(x)} \left[ \left\| \frac{1}{2} s_\theta(x) \right\|^2 + \, \text{Tr} \left( \nabla_x s_\theta(x) \right) \right] + C
\end{align}
